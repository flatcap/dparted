...
ErrorDialog::ErrorDialog (void) :
	MessageDialog ("", true, Gtk::MessageType::MESSAGE_ERROR, Gtk::ButtonsType::BUTTONS_NONE, false)
{
	add_button ("_Close", Gtk::ResponseType::RESPONSE_CLOSE);
	set_default_response (Gtk::ResponseType::RESPONSE_CLOSE);

	signal_response().connect(sigc::mem_fun(this,&ErrorDialog::on_dialog_response));
}
...
void
ErrorDialog::on_dialog_response (int button_id)
{
	log_debug ("Button: %d\n", button_id);
}
...
ErrorDialog::run (void)
{
	...
	if (!help_url.empty()) {
		...
		help.signal_clicked().connect (sigc::mem_fun (this, &ErrorDialog::on_help));
	}
