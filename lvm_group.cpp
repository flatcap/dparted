 ...
 */
void fd_lvs (DPContainer &disks) //RAR disks unused
{
	std::string command;
	std::vector<std::string> output;
	std::string error;
...
/**
 * fd_vgs2 - Create the LVMGroup objects
 */
std::vector<std::string>
fd_vgs2 (DPContainer &top_level, std::string &vol_name)
{
	LOG_TRACE;
...
/**
 * fd_pvs2 - Attach all the Segments (LVMGroup and Volumes)
 */
void fd_pvs2 (DPContainer &top_level, std::vector<std::string> devices)	//RAR top_level unused
{
	LOG_TRACE;
...
/**
 * fd_lvs2 - Build the Volumes from the Segments
 */
void fd_lvs2 (DPContainer &top_level, std::string vol_name) //RAR top_level unused
{
	LOG_TRACE;
...
LVMGroup::discover (DPContainer &top_level, LVMTable *t)
{
	...
	devices = fd_vgs2 (top_level, vol_name);
	...
	fd_pvs2 (top_level, devices);
	fd_lvs2 (top_level, vol_name);
}
