...
{
	m_checkbutton.signal_clicked().connect(sigc::mem_fun(*this, &ExampleWindow::on_markup_checkbutton_click));
	m_text_view.signal_query_tooltip().connect(sigc::mem_fun(*this, &ExampleWindow::on_textview_query_tooltip));
	m_button.signal_query_tooltip().connect(sigc::mem_fun(*this, &ExampleWindow::on_button_query_tooltip));
}
...
bool ExampleWindow::on_textview_query_tooltip(int x, int y, bool keyboard_tooltip, const Glib::RefPtr<Gtk::Tooltip>& tooltip)
{
	...
	if (keyboard_tooltip) {
		...
		m_text_view.window_to_buffer_coords(Gtk::TEXT_WINDOW_TEXT, x, y, mouse_x, mouse_y);
	...
	if (iter.has_tag(m_ref_bold_tag)) {
		char buffer[64];
		snprintf (buffer, sizeof(buffer), "<b>Tooltip</b> at (%d,%d)", x, y);
		//tooltip->set_markup("<b>Information</b> attached to a text tag");
		tooltip->set_markup(buffer);
